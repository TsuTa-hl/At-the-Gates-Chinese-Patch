...
这是 Jon Shafer's *At the Gates* 的非官方、免费、非商业简体中文补丁。补丁不包含原版游戏文件；请先安装合法拥有的游戏副本。
...
安装器只确认目录包含 `At The Gates.exe` 与 `Content\Text\English.xml`，不会检查游戏版本或文件指纹。因此它可以安装到玩家修改过或装有 MOD 的 AtG 目录。

安装前，脚本会备份每一个将被覆盖的实际文件到游戏目录中的 `_ChinesePatchBackup`，并写入事务清单。若补丁与 MOD 覆盖同一路径，MOD 文件会在安装期间被暂时覆盖；卸载后会按字节恢复。无关 MOD 文件不会被修改。
...
卸载会逐项验证并恢复安装前的备份。只有安装前不存在的补丁专有文件会被删除。若某个补丁专有文件在安装后被其他工具修改，脚本会停止并保留清单与备份，避免误删玩家内容。
